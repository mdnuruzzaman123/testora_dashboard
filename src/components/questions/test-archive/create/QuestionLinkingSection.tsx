...
export default function QuestionLinkingSection() {
  return (
    <section className="rounded-lg border border-[#dce7f2] bg-white p-4">
      ...
      <p className="mb-3 text-xs text-[#90a3b6]">Link questions from the Question Bank — no content is duplicated here</p>
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-4">
        <input className="rounded-md border border-[#dce7f2] bg-[#f8fbff] px-3 py-2 text-xs text-[#4f6d87] outline-none" placeholder="Category" />
        <input className="rounded-md border border-[#dce7f2] bg-[#f8fbff] px-3 py-2 text-xs text-[#4f6d87] outline-none" placeholder="Subject" />
        <input className="rounded-md border border-[#dce7f2] bg-[#f8fbff] px-3 py-2 text-xs text-[#4f6d87] outline-none" placeholder="Test Type" />
        <input className="rounded-md border border-[#dce7f2] bg-[#f8fbff] px-3 py-2 text-xs text-[#4f6d87] outline-none" placeholder="Status" />
      </div>
        ...
        <table className="w-full min-w-[800px] text-left">
          ...
          <tbody>
            {linkedQuestionsRows.map((row) => (
              <tr key={row.id} className="border-b border-[#ecf2f8] text-xs text-[#5e768e] last:border-b-0">
                <td className="px-3 py-2"><input type="checkbox" className="h-3.5 w-3.5 accent-[#2f86d8]" defaultChecked /></td>
                ...
                <td className={cn("px-3 py-2", row.passage !== "—" ? "text-[#8468c4]" : "text-[#90a3b6]")}>{row.passage}</td>
                <td className="px-3 py-2">
                  <span className={cn("rounded-full border px-2 py-0.5 text-[10px]", statusClass(row.status))}>{row.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
